...
export function add3D2PCommands(context: vscode.ExtensionContext) {
    ...
    let initProjectCommand = vscode.commands.registerCommand("3d2p.cmd.initProject", async () => {
        return commandEngine.start(new InitProjectCommand());
    });

    let setProjectDataCommand = vscode.commands.registerCommand("3d2p.cmd.setProjectData", async () => {
        return commandEngine.start(new SetProjectDataCommand());
    });

    let addStlCommand = vscode.commands.registerCommand("3d2p.cmd.addStl", async (fsPath: string) => {
        return commandEngine.start(new AddStlCommand(fsPath));
    });

    let setStlDataCommand = vscode.commands.registerCommand("3d2p.cmd.setStlInfo", async (stlInfo: StlInfo) => {
    ...
    let open3mfCommand = vscode.commands.registerCommand("3d2p.cmd.open3mf", async (uri: vscode.Uri) => {
        return commandEngine.start(new Open3MFCommand(uri.fsPath));
    });

    let openStlCommand = vscode.commands.registerCommand("3d2p.cmd.openStl", async (uri: vscode.Uri) => {
        return commandEngine.start(new OpenSTLCommand(uri));
    });
