...
export class SetProjectDataCommand extends BaseCliCommand {
    ...
    public thumbnailFilePath: FilePickQuestion = new FilePickQuestion("Select thumbnail (Skip, if don't want to change)", { 'Image': ['png', 'jpg', 'jpeg'] }, false);
    public readmeFilePath: FilePickQuestion = new FilePickQuestion("Select Readme  (Skip, if don't want to change)", { 'Markdown': ['md', 'txt' ] }, false);
    ...
    public async vscCommand(progress: vscodeProgress): Promise<CommandResult> {
        ...
        if(!result.isFaulted) {
            result.isFaulted = 
                !(await this._cli.setProjectData(
