...
export class AddStlCommand extends BaseCliCommand {
    public stlFilePath: FilePickQuestion = new FilePickQuestion(
        "Select STL",
        { STL: ["stl"] },
        true,
        (() => this._stlFilePath === undefined).bind(this)
    );
    ...
    public async vscCommand(progress: vscodeProgress): Promise<CommandResult> {
        ...
        let stlFilePath = this._stlFilePath !== undefined ? this._stlFilePath : this.stlFilePath.answer!;

        result.isFaulted = !(await this._cli.addStl(this._projectFilePath!, stlFilePath, this.status.answer!));
        result.message = result.isFaulted ? `Couldn't add STL! Please see 3D2P output for details.` : "STL successfully added.";
        return result;
    }
