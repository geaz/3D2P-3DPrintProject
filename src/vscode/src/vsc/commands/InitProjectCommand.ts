...
export class InitProjectCommand extends BaseCliCommand {
    ...
    public readmeFilePath: FilePickQuestion = new FilePickQuestion("Select Readme (Markdown)", { 'Markdown': ['md', 'txt' ] }, false, this.shouldCreateProject.bind(this));
    ...
    public async vscCommand(progress: vscodeProgress): Promise<CommandResult> {
        ...
        if(this.shouldCreateProject()) {
            ...
            result.message = result.isFaulted 
                ? `Couldn't create project file! Please see 3D2P output for details.` 
                : '';
